...
# An atomic Int
extern class AtomicVariable in "C" `{ int* `}
	new(i: Int)`{
...
private extern class NativePthread in "C" `{ pthread_t * `}
	...
	fun kill(signal: Int): Int `{ return pthread_kill(*self, signal); `}

	fun cancel `{ pthread_cancel(*self); `}
end
...
# Nit version of a condition variable
class PthreadCond
