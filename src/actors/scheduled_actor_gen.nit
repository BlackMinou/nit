...
# Generate actor classes for a module using "is actor" on its classes
# This module uses the scheduled actor model abstraction
module scheduled_actor_gen
...
private class ActorPhase
	...
	do
		...
		# Generate the superclass for all Message classes (each actor has its own Message super class)
...
""")
		...
		for m in methods do
			...
			# Params to send to `instance` in the `invoke` method of the Message class
			var params = ""
...
class {{{name}}}
	...
	redef fun invoke do {{{ret_val}}}instance.{{{method_name}}}{{{params}}}{{{ret_val2}}}
end
""")

			# The message knows at least the instance of the working class
			# So we add it to the parameters
			if params != "" then
				params = params.replace("(", "(instance,")
			else
				params = "(instance)"
			end
...
""")
		...
		# All of the functions of the proxy/actor too

		# Let's generate the proxy/actor class then
		actors.add(
"""
redef class Actor{{{classname}}}
	super Actor

	redef type V: {{{classname}}}

	{{{proxy_calls.join("\n\n")}}}
end
""")

		# Now redef the annotated classe to add the actor as "async" attribute
		redef_classes.add(
"""
redef class {{{classname}}}
	redef var async is lazy do return new Actor{{{classname}}}(self)
end
""")
	end
end
...
Usage: nitactor [OPTION] module.nit
Generates the boilerplate code for the nit scheduled actor model"""
