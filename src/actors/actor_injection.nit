...
# This module works for both scheduled and threaded actors, adding an option to the compiler
module actor_injection
...
redef class ToolContext
	# Actors injection option
	var actors_opt = new OptionString("Which actor model to inject, 'threaded' or 'scheduled'", "--actors_inject")

	init do self.option_context.add_option(self.actors_opt)

	redef fun process_options(args)
	do
		super

		if actors_opt.value == null then actors_opt.value = "threaded"
	end
end
...
redef class ModelBuilder
	...
	do
		...
		var injected_name: String
		if toolcontext.actors_opt.value == "threaded" then
			injected_name = "Proxy"
		else
			injected_name = "Actor"
		end
